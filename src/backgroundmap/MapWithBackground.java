...
import mapViewer.Layer;
import mapViewer.MapObject;
import mapViewer.Transformation;
 ...
 */
public class MapWithBackground extends JComponent implements MouseListener, MouseMotionListener {
     ...
     */
    protected TreeMap<Integer, Layer> layers;
    
    
    protected TreeMap<Integer, viewer.base.Layer> backgroundLayers;
    
    /**
     * The transformation that is used to transform map coordinates to image coordinates.
     */
    protected Transformation myTransformation;
     ...
     */
    private MapFrameWithBackground myMapFrame;
        
    /**
     * This constructor generates a new empty MapDisplay.
     */
        
    public MapWithBackground(MapFrameWithBackground m) {
        super();
        myMapFrame = m;
        ...
        backgroundLayers = new TreeMap<Integer, viewer.base.Layer>();
    ...
    public void paint(Graphics gSimple) {
		...
		geometry.Envelope e = new geometry.Envelope(x1, x2, y1, y2);
		Color oldColor = g.getColor();
		g.clearRect(0, 0, this.getWidth(), this.getHeight());
		for (viewer.base.Layer l : backgroundLayers.values()) {
			for (Object o : l.query(e)) {
				Symbol mo = (Symbol) o;
				mo.draw(g, new viewer.base.Transformation(
						myTransformation.getM(), 
						myTransformation.getColumnOrigin(), 
						myTransformation.getRowOrigin()));
			}
		}
		g.setColor(oldColor);
                
		//draw foreground
        Envelope e2 = new Envelope(x1, x2, y1, y2);
        oldColor = g.getColor();
      //  g.clearRect(0, 0, this.getWidth(), this.getHeight());        
        for (Layer l : layers.values()) {
        	g.setColor(l.getColor());
            for (Object o : l.query(e2)) {
